...
def _common_scope(*scopes):
    return '/'.join(part for part, in it.takewhile(
...
def _extract_valid_collections(
    ...
    try:
            ...
            for item in collection:
                ...
                try:
                    meta_graph_lib.import_scoped_meta_graph(
                        item_def, import_scope='import', graph=test_graph)
        ...
        meta_graph_def, _ = meta_graph_lib.export_scoped_meta_graph(
            graph=graph, graph_def=graph_def, **export_kw)
    finally:
...
def _extract_subgraph(input_ts, output_ts, graph, extra_dependencies=None):
    ...
    inputs_from_name.update({
        node.name: {_as_op_name(name) for name in node.input}
        for node in graph_def.node})

    # Add colocation nodes.
    for node in graph_def.node:
        inputs_from_name[node.name] |= set(map(
            _as_op_name, node.attr['_class'].list.s))
...
def extract_subgraph(
    ...
    if not output_list:
        ...
        all_tensors = {
            t for op in graph.get_operations() for t in op.outputs}
        tensors_which_has_output_ops = {
            t for op in graph.get_operations() for t in op.inputs}
        output_ts = list(all_tensors - tensors_which_has_output_ops)
    else:
        output_ts = [
            graph.get_tensor_by_name(_as_tensor_name(t))
...
class StaticGraph(tf.keras.layers.Layer):
    ...
    def __init__(self, inputs, outputs, *, training=None, updates=None,
        ...
        graph = get_graph_from_inputs(
            input_list + output_list + training_list + update_list + loss_list,
            graph)
        ...
        if self.__training:
            def _uses_lp(t, path=(), targets=self.__training):
                return t.name in targets or t.name not in path and any(
                    _uses_lp(ti, path + (t.name,)) for ti in t.op.inputs)
        ...
        self.__meta_graph_def, _, var_from_name = extract_subgraph(
            graph=graph, inputs=total_inputs, outputs=total_outputs)

        # Extract oprations other then the input tensors and the variables.
        (self.__unbound_meta_graph_def,
         self.__export_scope, _) = extract_subgraph(
            graph=graph, inputs=total_inputs, outputs=total_outputs,
            unwrap_scope=True, unbind_inputs=True, keep_variables=False)
        ...
        if kwargs.get('name') is None and default_name is not None:
            kwargs['name'] = _unique_layer_name(default_name)
    ...
    def build(self, input_shape):
        ...
        var_from_name = meta_graph_lib.import_scoped_meta_graph(
            variables_def, import_scope='variables')
    ...
    def call(self, inputs, training=None):
        ...
        def _import_scope(name):
            return _join_scope(
                import_scope, _rel_scope(name, self.__export_scope))

        # Save training operations.
        self.add_update(inputs=inputs, updates=[
            graph.get_operation_by_name(_import_scope(name))
            for name in self.__updates])

        self.add_loss(inputs=inputs, losses=[
    ...
    def weight_dict(self):
        return {_as_op_name(_rel_scope(name, self.__export_scope)): value
...
def as_layer(inputs, outputs, *, training=None, updates=None, losses=None,
