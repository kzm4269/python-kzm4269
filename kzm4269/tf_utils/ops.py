...
def nonbusy_device(use_cpu=False):
    ...
    def utilization(gpu_id):
        info = check_output(['nvidia-smi', f'--id={gpu_id}', '-q']).decode()
        return [
            int(re.findall(s + r'\s+: (\d+) %', info)[0])
            for s in ('Gpu', 'Memory', 'Encoder', 'Decoder')]
    ...
    if use_cpu:
        ...
        gpu_ids = re.findall(
            r'GPU (\d+):',
            check_output(['nvidia-smi', '-L']).decode())
        device_name = '/gpu:' + min(gpu_ids, key=utilization)
    ...
    with tf.name_scope(name, default_name, [y, x]):
        ...
        def body(i, res):
            with tf.name_scope('body'):
                ...
                return tf.reshape(tf.concat([head, tail], axis=axis), y_shape)

        _, result = tf.while_loop(
            lambda i, _: i > 0,
            lambda i, res: (i - 1, body(i, res)),
            [tf.constant(y_shape[axis], tf.int32), y])
        return result


def rdiff(y, x=None, axis=-1, name=None):
    return _diff_recursively(
        y, x, axis=axis, inverse=False,
        default_name='rdiff', name=name)


def irdiff(y, x=None, axis=-1, name=None):
    return _diff_recursively(
        y, x, axis=axis, inverse=True,
        default_name='irdiff', name=name)
